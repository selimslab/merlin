...
class PageRank:
    ...
    def get_cosine_similarity_scores(self, index, number_of_pages, query):
        ...
        query_tf_idfs = {
            term: self.scorer.get_tf_idf(1, number_of_pages, len(index[term]))
    ...
    def teleport(self, number_of_pages, transition_matrix):
        return (
                transition_matrix * (1 - self.teleport_rate)
                + self.teleport_rate / number_of_pages
        )
    ...
    def create_page_rank(self, pages):
        ...
        page_rank = {url: round(rank, 3) for url, rank in zip(urls, ranks)}
        return page_rank
...
class SearchEngine:
    ...
    def search(self, query):
        cosine_similarity_scores = self.ranker.get_cosine_similarity_scores(
            self.index, self.number_of_pages, query
        )
        logging.debug(f"cosine_similarity_scores {cosine_similarity_scores}")
        final_scores = [
            (url, score * self.page_rank.get(url))
            for url, score in cosine_similarity_scores
        ]
